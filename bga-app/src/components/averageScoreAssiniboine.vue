...
<v-simple-table class="elevation-1" fixed-header height=300>
    <template v-slot:default>
        ...
        <tbody>
            <tr v-for="player in averagesForAssiniboine" :key="player.playerId">
...
export default {
    ...
    computed:{
        averagesForAssiniboine(){
            var course = []
            for(let i = 0; i<this.scores.length; i++){
                if(this.scores[i].courseName === 'Assiniboine Golf Club'){
